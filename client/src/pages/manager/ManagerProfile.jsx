  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Fetch franchise info if manager has a franchise assigned
        if (userData.franchise) {
          try {
            const token = localStorage.getItem('token');
            const response = await axios.get(
              `${process.env.REACT_APP_API_URL}/api/franchises/${userData.franchise}`,
              {
                headers: {
                  ...
                  <div className="w-32 h-32 rounded-full overflow-hidden border border-gray-300">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.src = "https://ui-avatars.com/api/?name=" + encodeURIComponent(profileData.name || "User");
                      }}
                    />
                  </div>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-medium">{user?.role === 'orderManager' ? 'Order Manager' : user?.role || 'Manager'}</p>
            </div>
            <div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{franchiseInfo.name || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Location</p>
                <p className="font-medium">
                  {franchiseInfo.address && typeof franchiseInfo.address === 'object'
                    ? `${franchiseInfo.address.Street || ''}, ${franchiseInfo.address.city || ''}, ${franchiseInfo.address.state || ''}, ${franchiseInfo.address.pincode || ''}`
                    : franchiseInfo.address || 'N/A'}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Contact Number</p>
                <p className="font-medium">{franchiseInfo.contactNumber || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Email</p>
                <p className="font-medium">{franchiseInfo.email || 'N/A'}</p>
              </div>
            </div>
          </div>
        )}
